...
knitr::kable(Phylla, caption = "Number of occurrences for each Phylum present in descending order", format = "latex", row.names = FALSE, format.args = list(big.mark = ","))
```

The final dataset had `r prettyNum(nrow(All2), big.mark = ",")` occurrences, bellonging to `r prettyNum(length(unique(All2$phylum)), big.mark = ",")` Phylla (Table 1) and `r prettyNum(length(unique(All2$species_name)), big.mark = ",")` species
